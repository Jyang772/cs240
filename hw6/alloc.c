...
void freef(char *p)
{
	...
	if((char*)rb >= allocbuf && rb->tag == FREETAG) {
		temp = (struct blockl*)(p - rb->size);	//point to left block of block to the left
		p -= rb->size;
		coalesce(temp,lb);
		lb=temp;
	}
	else	//if no free block to the left, then add q(p) to free list.
		enchain(lb);
	...
	if((char*)temp < allocbuf + ALLOCSIZE && temp->tag == FREETAG) {
		unchain(temp);	//unchain temp, because it is now part of a larger block
		coalesce(lb,temp); //combine block with block to the right
	}
}	
